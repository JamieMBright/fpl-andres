...
| A12 | 7     | Do             | Neither Wildcard is ever played: both score `gain = 0.00`, and the note says why -- "nothing in this projection breaks". Projections are one static per-match number varied only by fixture, so the squad never drifts and a chip whose value is repairing drift cannot score. The segmentation that lets a Wildcard truncate the planning horizon is built and therefore dead. Needs A9's predictive distribution before it can fire.                                                                                                                                      |
| A13 | 5     | **Done**       | Two builds of the same fifteen disagreed about the bench. A Wildcard rebuild valued it at zero; the opening squad valued the reserve keeper at `1 - P(starting keeper appears)`, which bought a £6.0m reserve who never plays. Gameweek 1 is itself a free rebuild, so the two had no reason to differ. The reserve keeper is now worth zero; outfield auto-sub weights are measured and unchanged.                                                                                                                                                                         |
