...
| A5  | 8     | Do             | Nine captaincy comparisons, no multiplicity correction. Family-wise error near 37%. No false positive published yet; the instrument will produce one.                                                                                               |
...
| A8  | 5     | Owner decision | The mini-league baselines are weak. Beating a manager who never transfers is not evidence. A competent-template policy is buildable.                                                                                                                |
...
| #   | Score | Verdict        | Finding                                                                                                                                                                                                                                      |
| --- | ----- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B1  | 9     | Do             | **Verified.** No caller passes `team_id` to `project_element_rates`, so `_carried_context` always returns `"unknown"` and the 0.6 club-change discount is unreachable. Tests pass because they bypass the projector.                         |
| B2  | 8     | Do             | **Verified.** `clean_sheet_rate` and `bonus` are the only unshrunk routes in `supporting_breakdown`. Three lines below, saves carries the comment "Shrunk like every other route". Clean sheets are 13% of all points, bonus 6.4%.           |
