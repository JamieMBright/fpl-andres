...
test("FPL500 headlines and the legal popularity squad fit a phone", async ({
  ...
  // The gameweek just captured has no scored points until FPL confirms them,
  // and the ledger shows an em dash until it does. Naming "59 raw" pinned the
  // page to gameweek one; requiring a number would redden it every time a
  // round is captured before it is scored.
  await expect(page.getByText(/(\d+|—)\s*raw/)).toBeVisible();
