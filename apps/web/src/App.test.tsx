...
describe("team analysis entry", () => {
  ...
  it("stores manager corrections separately against the public deadline", async () => {
    ...
    const corrections = screen
      .getByRole("heading", { name: "Manager Corrections" })
      .closest<HTMLElement>(".correction-panel-body");
    expect(corrections).not.toBeNull();
    await user.type(screen.getByLabelText("Current bank (£m)"), "1.2");
    const availableTransfers = within(corrections!).getByRole("combobox", {
      name: "Available free transfers",
    });
    expect(
      within(availableTransfers)
        .getAllByRole("option")
        .map((option) => ({
    ...
    await user.selectOptions(availableTransfers, "2");
    const chipGroup = within(corrections!).getByRole("group", {
      name: "Available chips",
    });
    const chipGrid = chipGroup.querySelector(".correction-chip-grid");
    expect(chipGrid).not.toBeNull();
    expect(chipGrid?.children).toHaveLength(4);
    await user.click(
      within(corrections!).getByRole("checkbox", { name: "Wildcard" }),
    );
    await user.click(
      within(corrections!).getByRole("checkbox", { name: "Bench Boost" }),
    );
    await user.click(
