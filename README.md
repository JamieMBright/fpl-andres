...
| Bookmaker odds          | Ingested on a GitHub runner only — the owner's network blocks every price host at the TLS handshake. Team scorelines come from 1X2 and over/under under independent Poisson; player goals, assists, cards and shot lines are route evidence. Missing markets leave the historical route standing. |
...
- The odds ingest and the player-market survey run on GitHub runners only,
  ...
  ahead of refreshing an existing quote.
