...
`OptimizationError` and `ModelFitError` are in this class rather than *Degrade*
...
The distinction from *Refuse* is whether the absence is **expected**. Picks are
...
retry within the bounds the module documents, then escalate to *Refuse* if the
...
It is here rather than in *Refuse* because nothing is broken.
...
is exactly the reason not to have one: it invites catching *Refuse* and
*Degrade* in the same clause, which is the mistake this taxonomy exists to
prevent.
