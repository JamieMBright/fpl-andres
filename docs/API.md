...
| `no_processed_event` | FPL has not processed a gameweek yet. Normal between seasons: `/entry/{id}/` returns `current_event: null` after the reset. | An explanation that the season has not started, plus the manager's own record, which is still real. |
