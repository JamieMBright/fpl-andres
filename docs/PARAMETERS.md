...
| xStart candidate half-life grid   | 2 / 4 / 8                        | `cli/experiment_xstart.py`             | Predeclared tuning candidates, selected on 2022/23–2023/24 only.                                                                                                                                                                                                                                                                                       |
| xStart candidate prior grid       | 1 / 2 / 4                        | `cli/experiment_xstart.py`             | Predeclared tuning candidates, selected on 2022/23–2023/24 only.                                                                                                                                                                                                                                                                                       |
| recent-form-weight candidate grid | 0.10 / 0.15 / 0.20 / 0.25 / 0.30 | `cli/experiment_recent_form_weight.py` | Production-disabled candidates selected on 2022/23–2023/24; family-corrected paired MAE and a per-season Spearman guardrail are held out on 2024/25–2025/26.                                                                                                                                                                                           |
...
| `WILDCARD_HORIZONS`               | 3 / 5 / 7 / 9                    | `cli/publish_season_plan.py`           | Legal squad horizons compared to expose a persistent-team cliff before exact whole-season Wildcard re-planning.                                                                                                                                                                                                                                        |
| `WILDCARD_CANDIDATES`             | 2 per half                       | `cli/publish_season_plan.py`           | Highest horizon-cliff weeks given an exact whole-season re-plan; bounded so publication completes.                                                                                                                                                                                                                                                     |
| `MINIMUM_WILDCARD_CHANGES`        | 5                                | `cli/publish_season_plan.py`           | A permanent rebuild below this is ordinary transfer work, not a Wildcard.                                                                                                                                                                                                                                                                              |
| `MINIMUM_FREE_HIT_CHANGES`        | 10                               | `cli/publish_season_plan.py`           | Recommendation threshold for a broad xPts1 rental. An explicit manager commitment is still modeled even when its best playable squad overlaps more heavily.                                                                                                                                                                                            |
| season-plan solve proof limit     | 120 seconds per solve            | `validate-model.yml`                   | HiGHS must prove each lexicographic optimum; the horizon-cliff screen limits how many full-season candidates receive this budget.                                                                                                                                                                                                                      |
| browser Wildcard horizons         | 3 / 5 / 7 / 9                    | `state/season-chips.ts`                | Manager-specific rebuild advice compares each legal horizon against the already solved future plan.                                                                                                                                                                                                                                                    |
| browser rebuild passes            | 2                                | `pages/SeasonPlanPage.tsx`             | First solve establishes the no-chip counterfactual; the second applies the exact advised Free Hit/Wildcard squads and replans every following gameweek.                                                                                                                                                                                                |
| fixture difficulty midpoint/scale | 3.0 / 2.18                       | `planning/fixture_routes.py`           | Opponent attack over defensive tightness at the opponent's venue, mapped logarithmically to 1–5. This summary never moves route-specific xPts.                                                                                                                                                                                                         |
...
| `current_season_weight`            | 4.0         | `backtesting/projector.py`        | Weight per settled current-season lineup against carried appearances. Selected on 2022-24 and held out on 2024-26; paired Brier improvement 0.0497, 95% lower bound 0.0348, 3/3 seeds promoted.       |
