...
| `--market-weight`                | 0.35        | `cli/publish_season_inputs.py`    | How much of a player's goal and assist expectation a bookmaker owns. Assumed, not measured: there is no player-prop history here to fit it against, and it decides how far a quoted price can move a projection. |
...
with. Kept as one flag in one place so it can be moved when there is.
