...
| `SHORTLIST_SIZE`                 | 25          | `backtesting/captaincy.py`        | How many players a captaincy thesis may choose between.                                                                                                                                                                      |
...
| `--market-weight`                | 0.35        | `cli/publish_season_inputs.py`    | How much of a player's goals, assists, bookings and start rate a bookmaker owns. Assumed, not measured: there is no player-prop history here to fit it against, and it decides how far a quoted price can move a projection. |
| `CLUB_QUOTE_FLOOR`               | 11          | `cli/publish_season_inputs.py`    | How many of a club's players a book must have priced before its silence about one of them counts as a team sheet. Assumed: eleven is a starting side, not a measurement of how books open markets.                           |
...
governs the card routes and the start-rate cut as well, which makes it worth
more than it was: one unmeasured number deciding four things.
